...
def simulate_perfect_model(N=1000):
    y_true = np.random.randint(0, 3, size=N)
    y_pred = y_true.copy()
    y_conf = np.ones(N)  # full confidence
...
if __name__ == "__main__":
    ...
    y_true, y_pred, y_conf = simulate_perfect_model()
    df = evaluate(y_true, y_pred, y_conf, thresholds)
    df.to_csv("results/synthetic_perfect.csv", index=False)
    print("Saved: results/synthetic_perfect.csv")
